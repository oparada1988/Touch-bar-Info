...
DEFAULT_PRESET = {
    ...
    # Background & Dividers
    "custom_bg_path": "/mnt/Stuff/Streamdeck Icons/bitmap2.png",
...
class TouchBarInfoAction(ActionBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Check for first-run unconfigured state to apply Master Preset via GTK idle (after Action registration)
        def _apply_first_run_preset():
            try:
                ...
                if settings is not None and not settings:
                    ...
                    preset["custom_bg_path"] = bg_path
                    self.set_settings(preset)
                    self.trigger_redraw()
                    log.info("TouchPulse: First-run master preset successfully applied to page configuration!")
                ...
                log.warning(f"TouchPulse: Error applying first-run master preset in idle: {e}")
            return False

        GLib.idle_add(_apply_first_run_preset)
    ...
    def get_slot_setting(self, settings: dict, slot_key: str, sub_key: str, default):
        ...
        if full_k in DEFAULT_PRESET:
            return DEFAULT_PRESET[full_k]
        if sub_key in DEFAULT_PRESET:
            return DEFAULT_PRESET[sub_key]
        return default
